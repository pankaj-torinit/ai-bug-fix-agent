...
async function processBug(job) {
  ...
  const workspaceId = `${job.id}-${eventId}`;
  ...
  try {
    ...
    const cloneResult = await repoService.cloneRepo(repoUrl, workspaceId);
    ...
    if (repoPath && fs.existsSync(repoPath)) {
      try {
        ...
        console.log('[Worker] Removed job workspace');
      } catch (err) {
        console.warn('[Worker] Failed to remove workspace:', err.message);
      }
    }
  }
}
...
const worker = new Worker(
  ...
  {
    connection,
    /** Each job uses its own clone dir under TEMP_REPO_PATH; raising this is safe (W10). */
